...
import { collection, addDoc, setDoc, doc,getDocs,getDoc,query, where, Query, CollectionReference, QuerySnapshot, updateDoc,deleteDoc } from "firebase/firestore";
...
export async function deleteAllFromUser(email, userId) {
  ...
  //console.log(userAuth.currentUser.email); 
  // Checks if the user is admin or superAdmin

  if (await isAdmin(userAuth.currentUser.email) || await isSuperAdmin(userAuth.currentUser.email)) {
    if (await isAdmin(userAuth.currentUser.email) && (await isAdmin(email) || await isSuperAdmin(email))){
      console.error("Unable to delete a admin or superadmin");
    }else{  
      try {
            ...
            },
            {
              ...
              onPress: async () => {
                ...
                await Promise.all([
                  ...
                  deleteUser(userAuth, uidToDelete), // Include deleteUser function in Promise.all
                ]);
              },
            },
          ],
